...
export function RegisterRunnerDialog() {
  ...
  const registrationCommand = state.token
    ? `npx @deployx/runner register --token ${state.token} --url ${window.location.origin}`
    : "";
            ...
            {!state.token ? (
              ...
              <>
                <p className="text-sm text-on-surface-variant mb-4">
                  Runner registered! Copy the command below and run it on your
                  target host. The token is shown{" "}
                  <span className="text-error font-bold">only once</span>.
                </p>

                <div className="bg-surface-container-lowest border border-outline-variant/20 rounded-md p-4 mb-6">
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <button
                      onClick={() => handleCopy(registrationCommand)}
